...
function StatusView(props: { api: Parameters<TuiPlugin>[0] }) {
  ...
  const activeTier = () => {
    ...
    if (profile.tier === "local" || profile.provider === "local") return "Local"
    if (profile.tier === "free") return "OpenRouter free"
    if (profile.tier === "paid") return "OpenRouter paid"
  ...
  const activeModel = () => {
    const profile = status()?.deployment?.active?.profile
    if (!profile) return status()?.provider ?? null
  ...
  const detail = () => {
    const s = status()
    if (!s || s.status === "unavailable") return null
