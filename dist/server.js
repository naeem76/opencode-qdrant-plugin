...
import { consumeReindexTrigger, readStatusFile, writeStatusFile, } from "./status-file.js";
...
const server = async (input, rawOptions) => {
    ...
    const refreshStoredOpenRouterKey = async () => {
        if (!credentialLoadedFromOpenCode && process.env.OPENROUTER_API_KEY?.trim())
            return;
        ...
        if (state.status !== lastToastStatus) {
            ...
            switch (state.status) {
                ...
                case "rate_limited":
                    ...
                    break;
                case "complete":
                    await toast(`Index ready (${state.collectionPointCount ?? 0} chunks)`, "success", 4000);
                    break;
                case "error":
    ...
    }, log, settings?.fallbackToLocalOnRateLimit ?? false);
    let fileWatcher = null;
    const ensureWatcher = () => {
        if (fileWatcher || !options.watchFiles || !manager.hasActiveIndex())
            return;
        fileWatcher = startFileWatcher({
            rootDirectory: input.directory,
            debounceMs: options.watchDebounceMs,
            onChange: async (paths) => {
                if (!(await manager.healthCheck()))
                    return;
