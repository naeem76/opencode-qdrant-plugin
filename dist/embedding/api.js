...
export class ApiEmbeddingProvider {
    ...
    constructor(options) {
        ...
        this.requestIntervalMs = Math.max(0, options.scheduler?.requestIntervalMs ??
    ...
    enqueue(texts, group, prioritized) {
    ...
    pump() {
        if (this.pumpTimer || this.queue.length === 0)
            return;
        const now = Date.now();
        ...
        return retryAsync(async () => {
            ...
            for (const vector of result) {
                if (!Array.isArray(vector) ||
                    ...
                    throw new Error(`Embedding API returned an invalid vector (expected ${this.options.dimensions} finite dimensions)`);
                }
            }
            return result;
